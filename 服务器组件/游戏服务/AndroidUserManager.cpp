...
IAndroidUserItem * CAndroidUserManager::CreateAndroidUserItem(tagAndroidParameter & AndroidParameter)
{
	...
	//变量定义
	STR_SUB_CG_LOGON_USERID LogonUserID;
	ZeroMemory(&LogonUserID,sizeof(LogonUserID));

	//版本信息
	/* added by WangChengQing 2018/1/17 好像并没有对version做校验
	...
	//LogonUserID.dwGameID=m_pGameServiceOption->dwGameID;

	//构造数据
	TCP_Command Command;
	Command.wMainCmdID=MDM_GR_LOGON;
	Command.wSubCmdID=SUB_CG_LOGON_USERID;
...
bool CAndroidUserManager::OnEventTimerPulse(DWORD dwTimerID, WPARAM dwBindParameter)
{
	...
	//		CAndroidUserItemMap tempUserItemMap;
	//		POSITION Position = m_AndroidUserItemMap.GetStartPosition();
	//		while (Position!=NULL)
	//		{
	//			m_AndroidUserItemMap.GetNextAssoc(Position,dwUserID,pAndroidUserItem);
	//			tempUserItemMap.SetAt(dwUserID, pAndroidUserItem);
	//		}

	//		
	//		Position = tempUserItemMap.GetStartPosition();
	//		//用户处理
	//		while (Position!=NULL)
	//		{
	//			//获取对象
	//			tempUserItemMap.GetNextAssoc(Position,dwUserID,pAndroidUserItem);

	//			//时间处理
	//			try
	//			{
	//				ASSERT(pAndroidUserItem!=NULL);
	//				if (dwUserID > 0 && pAndroidUserItem != NULL && pAndroidUserItem->m_pIServerUserItem != NULL)
	//				{
