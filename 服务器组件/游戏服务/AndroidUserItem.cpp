...
//获取定时器剩余毫秒数
DWORD CAndroidUserItem::GetTimerLeftTickCount(UINT nTimerID)
{
	//删除时间
	if (nTimerID!=0)
	{
		...
		for (INT_PTR i=0;i<m_TimerItemActive.GetCount();i++)
		{
			...
			if (pTimerItem->nTimerID==nTimerID)
			{
				return GetTickCount() - pTimerItem->nTimeLeftTickCount;
			}
		}
	}
...
bool CAndroidUserItem::SetGameTimer(UINT nTimerID, UINT nElapse)
{
	...
	for (INT_PTR i=0;i<m_TimerItemActive.GetCount();i++)
	{
		...
		if (pTimerItem->nTimerID==nTimerID)
		{
			...
			pTimerItem->nTimeLeftTickCount = GetTickCount();
			return true;
		}
	}
...
//	ASSERT((m_pIServerUserItem!=NULL)&&(m_pIServerUserItem->GetUserStatus()==US_SIT));
	...
	//发送准备
	DWORD dwAndroidID=MAKELONG(m_wAndroidIndex,m_wRoundID);
	m_pIAndroidUserManager->SendDataToServer(dwAndroidID,MDM_G_FRAME,SUB_GF_USER_READY,pData,wDataSize);
...
bool CAndroidUserItem::SendChatMessage(DWORD dwTargetUserID, LPCTSTR pszChatString, COLORREF crColor)
{
	...
	//m_pIAndroidUserManager->SendDataToServer(dwAndroidID,MDM_G_FRAME,SUB_GF_USER_CHAT,&UserChat,wSendSize);
...
bool CAndroidUserItem::OnTimerPulse(DWORD dwTimerID, WPARAM dwBindParameter)
{
	...
	for (INT_PTR i=0;i<m_TimerItemActive.GetCount();)
	{
		...
		if (pTimerItem->nTimeLeave<=1L)
		{
			...
			timers.Add(pTimerItem->nTimerID);
		}
		else
		{
			//设置变量
	...
	for (INT_PTR i=0; i<timers.GetCount(); i++)
	{
		//时间通知
		if (m_pIAndroidUserItemSink!=NULL)
		{
			m_pIAndroidUserItemSink->OnEventTimer(timers[i]);
		}
	}
...
bool CAndroidUserItem::OnSocketRead(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	//用户状态
	if ((wMainCmdID==MDM_USER)&&(wSubCmdID==SUB_GR_USER_STATUS))
	{
		return OnSocketSubUserStatus(pData,wDataSize);
	}

	//游戏消息
	if ((wMainCmdID==MDM_GF_GAME)||(wMainCmdID==MDM_G_FRAME))
	{
...
bool CAndroidUserItem::OnSocketGameMessage(WORD wMainCmdID, WORD wSubCmdID, VOID * pData, WORD wDataSize)
{
	...
	//框架处理
	if (wMainCmdID==MDM_G_FRAME)
	{
		//默认处理
		switch (wSubCmdID)
		{
		case CMD_GR_FRAME_GAME_STATUS:		//游戏状态
			{
				//效验参数
		...
		case CMD_GR_FRAME_GAME_OPTION:			//游戏场景
			{
				//游戏处理
				if (m_pIAndroidUserItemSink!=NULL)
				{
...
bool CAndroidUserItem::OnSocketSubLogonFinish(VOID * pData, WORD wDataSize)
{
	...
	//发送规则
	DWORD dwAndroidID=MAKELONG(m_wAndroidIndex,m_wRoundID);
	m_pIAndroidUserManager->SendDataToServer(dwAndroidID,MDM_USER,SUB_CG_USER_RULE,&UserRule,sizeof(UserRule));
...
VOID CAndroidUserItem::StartGameClient()
{
	...
	//发送数据
	DWORD dwAndroidID=MAKELONG(m_wAndroidIndex,m_wRoundID);
	m_pIAndroidUserManager->SendDataToServer(dwAndroidID,MDM_G_FRAME,SUB_RG_FRAME_OPTION,&GameOption,sizeof(GameOption));
