...
bool CServerListManager::InsertGameNode(tagGameNode * pGameNode)
{
	...
	//1. 为去除数据库中PlatformDB 中GameNodeItem(表)中NodeID的唯一key
	//2. 修改视图为层级结构,取消混乱处理, 使KindID为唯一标志,
	// 特 修改m_NodeItemMap 
...
//插入房间 -- 返回值false表示不需要通知client,  ture表示需要通知客户端
byte CServerListManager::InsertGameServer(tagGameServer * pGameServer)
{
	...
	//获取子项
	if (m_ServerItemMap.Lookup(pGameServer->wServerID,pGameServerItem)==FALSE) //新增子项
	{
		//创建对象
		try
		{
