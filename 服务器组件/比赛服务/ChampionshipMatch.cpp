	...
	if (m_enMatchStatus==MS_FIRST_ROUND)
	{
		...
		if (nCount<=(INT_PTR)m_pMatchRule->wRemainUserCount)
		{
			if(nCount<(INT_PTR)m_pMatchRule->dwSecRoundUserCount)
			{
				...
				m_pMatchSink->SendGroupData(MDM_G_FRAME,CMD_GF_SYSTEM_MESSAGE,&Message,wSendSize, this);
			...
			_sntprintf_s(szMsg,CountArray(szMsg),TEXT("已经达到淘汰人数，%d秒后即将结束初赛排名，确定晋级玩家！"),m_pMatchRule->wWaitTime);
	...
	else
	{
		...
		if(pTableFrame->pTableFrame->GetGameStatus()<GAME_STATUS_PLAY)
		{
			...
			while(pos!=NULL)
			{
				...
				{									
					m_pMatchSink->SendData(pUser,MDM_G_FRAME, SUB_GF_USER_READY, NULL,0);
...
bool CChampionshipMatch::OnEventGameStart(ITableFrame *pITableFrame, WORD wChairCount)
{
	...
	if(m_enMatchStatus==MS_SECOND_ROUND)
	{
		...
		{		
			...
			if(table.pTableFrame==pITableFrame)
			{
				...
				_sntprintf_s(szMsg, CountArray(szMsg),TEXT("淘汰赛第%d轮（共%d轮）第%d局开始（共%d局)。"),m_wCurRountSec,
				...
				//发送数据
				WORD wSendSize=sizeof(Message)-sizeof(Message.szString)+Message.wLength*sizeof(TCHAR);
				pITableFrame->SendTableData(INVALID_CHAIR,CMD_GF_SYSTEM_MESSAGE, (void*)&Message,wSendSize,MDM_G_FRAME);


				//构造数据
				CMD_GR_Match_Info MatchInfo;
				ZeroMemory(&MatchInfo,sizeof(MatchInfo));
				_sntprintf_s(MatchInfo.szTitle[0], CountArray(MatchInfo.szTitle[0]),TEXT("积分方式：积分"));
				_sntprintf_s(MatchInfo.szTitle[1], CountArray(MatchInfo.szTitle[1]),TEXT("游戏基数：%d"), m_dwCurBase);
				_sntprintf_s(MatchInfo.szTitle[2], CountArray(MatchInfo.szTitle[2]),TEXT("局制名称：%d局积分制"),m_pMatchRule->cbSecGameCount);
				_sntprintf_s(MatchInfo.szTitle[3], CountArray(MatchInfo.szTitle[3]),TEXT("%d局积分最高者获胜"), m_pMatchRule->cbSecGameCount);
				...
				for(WORD i=0; i<wChairCount; i++)
				{
					...
					//发送数据
					pITableFrame->SendTableData(i, SUB_GR_MATCH_INFO, &MatchInfo, sizeof(MatchInfo), MDM_G_FRAME);
				}
				break;
			}
		}
	...
	else if(m_enMatchStatus==MS_FIRST_ROUND)
	{
		...
		CMD_GR_Match_Info MatchInfo;
		ZeroMemory(&MatchInfo,sizeof(MatchInfo));
		_sntprintf_s(MatchInfo.szTitle[0], CountArray(MatchInfo.szTitle[0]),TEXT("积分方式：底分×基数"));
		_sntprintf_s(MatchInfo.szTitle[1], CountArray(MatchInfo.szTitle[1]),TEXT("游戏基数：%d"), m_dwCurBase);
		if(m_pMatchRule->enFirstRule==FirstRoundRule_Score)
		{
			_sntprintf_s(MatchInfo.szTitle[2], CountArray(MatchInfo.szTitle[2]),TEXT("局制名称：打立出局"));
			_sntprintf_s(MatchInfo.szTitle[3], CountArray(MatchInfo.szTitle[3]),TEXT("低于 %d 分将被淘汰"), (int)(m_dwCurBase*m_pMatchRule->cbLeastScore/100));
		}
		else
		{
			_sntprintf_s(MatchInfo.szTitle[2], CountArray(MatchInfo.szTitle[2]),TEXT("局制名称：定时排名"));
			CTimeSpan span=CTime::GetCurrentTime()-m_tMatchStart;
			_sntprintf_s(MatchInfo.szTitle[3], CountArray(MatchInfo.szTitle[3]),TEXT("大约 %d 分钟后排名"), 
	...
	case IDI_NORMAL_TIMER:
		{
			if(m_pMatchRule->enFirstRule==FirstRoundRule_Time)
			{
				...
				if(m_pMatchRule->dwFirstRoundTime-span.GetTotalSeconds()<=m_pMatchRule->dwAheadStatistics && !m_bWillOver)
				{
					...
					//发送数据
					WORD wSendSize=sizeof(Message)-sizeof(Message.szString)+Message.wLength*sizeof(TCHAR);
					return m_pMatchSink->SendGroupData(MDM_G_FRAME,CMD_GF_SYSTEM_MESSAGE,&Message,wSendSize, this);
...
void CChampionshipMatch::SelectPromotionUser()
{
	...
	for (INT_PTR i=0;i<nCount;i++)
	{
		if(i>=(INT_PTR)m_pMatchRule->dwSecRoundUserCount)
		{
			...
			m_pMatchSink->SendData(pScore[i].pUserItem,MDM_G_FRAME,SUB_GR_MATCH_WAIT_TIP,NULL,0);
		}
		else
		{
...
bool CChampionshipMatch::IsCanStanUp(IServerUserItem *pUserItem)
{
	if(m_enMatchStatus==MS_FIRST_ROUND || m_enMatchStatus==MS_STOP_OUT || m_enMatchStatus==MS_SIGNUP)
	{
		if(!m_bIsCanStandUp)
		{
			...
			{
				m_pMatchSink->SendData(pUserItem,MDM_G_FRAME, SUB_GF_USER_READY, NULL,0);
	...
	if(pITableFrameEx->cbGameCount>=m_pMatchRule->cbSecGameCount)
	{
		...
		if(m_wCurRountSec!=m_pMatchRule->cbSecRoundCount)
		{
			for(WORD i=0;i<m_wChairCount && i<MAX_CHAIR;i++)
			{
				...
				{
					TCHAR szMsg[256]=TEXT("");
					_sntprintf_s(szMsg,CountArray(szMsg),TEXT("您已完成本轮比赛，处于本桌第 %d 名。很遗憾，您被淘汰出局。请等待其他桌结束后重新排名确定所有淘汰选手名字。"),m_wChairCount);
				...
				{
					m_pMatchSink->SendData(pMatchScore[wMaxInx].pUserItem, MDM_G_FRAME, SUB_GF_USER_READY, NULL,0);
				}
				else
				{										
					...
					m_pMatchSink->SendData(pMatchScore[i].pUserItem, MDM_G_FRAME, SUB_GF_USER_READY, NULL,0);
				}				
			}
			...
			else //下一轮
			{
				...
				for (INT_PTR i=0;i<nMaybeCount;i++)
				{
					...
					else
					{
						...
						_sntprintf_s(szMsg, CountArray(szMsg),TEXT("很遗憾，您未能成功晋级！在本次比赛中获得第%d名。"), m_pMatchRule->arrSecRoundUserCount[m_wCurRountSec]+i+1);
						m_pMatchSink->SendGameMessage(pScore[i].pUserItem, szMsg, SMT_CHAT|SMT_TABLE_ROLL|SMT_CLOSE_GAME|SMT_EJECT);
						m_pMatchSink->SendData(pMatchScore[wMaxInx].pUserItem,MDM_G_FRAME,SUB_GR_MATCH_WAIT_TIP,NULL,0);
						tagMatchScore *pOutUser=new tagMatchScore;
						ZeroMemory(pOutUser,sizeof(tagMatchScore));
				...
				for (INT_PTR i=0;i<nOutUserCount;i++)
				{
					...
					if(ppOutUser[i]->pUserItem!=NULL)
					{
						...
						m_pMatchSink->SendData(ppOutUser[i]->pUserItem,MDM_G_FRAME,SUB_GR_MATCH_WAIT_TIP,NULL,0);
					}
				}
				for (INT_PTR i=0;i<m_OutUserArray.GetCount();i++)
				{
		...
		if(pUserItem!=NULL)
		{
			...
			if(m_enMatchStatus==MS_FIRST_ROUND)
			{
				...
				if (pScore->lScore<m_dwCurBase*m_pMatchRule->cbLeastScore/100|| bCanOut)
				{
					...
					_sntprintf_s(szReason, CountArray(szReason), TEXT("很遗憾，由于您的当前积分不足基数的%d%%，您被淘汰出局了。在本次比赛中获得第%d名。"),
...
VOID CChampionshipMatch::SendWaitTip(ITableFrame *pTableFrame)
{
	...
	if(m_enMatchStatus==MS_STOP_OUT || m_enMatchStatus==MS_SECOND_ROUND)
	{
		for(WORD i=0;i<m_wChairCount && i<MAX_CHAIR;i++)
		{
			...
			m_pMatchSink->SendData(pTableUserItem,MDM_G_FRAME,SUB_GR_MATCH_WAIT_TIP,&WaitTip,sizeof(WaitTip));
		}
	}
