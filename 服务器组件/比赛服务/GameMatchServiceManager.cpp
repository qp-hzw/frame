	...
	case IDI_START_MATCH_START+2:
		{
			...
			SendGroupData(MDM_G_FRAME,CMD_GF_SYSTEM_MESSAGE,&Message,wSendSize,pMatch);
				...
				for (INT_PTR i=0;i<nCount;i++)
				{
					...
					if(pTempMatch==pOverMatch)
					{
						for (WORD j=0;j<pOverMatch->m_wTableCount;j++)
						{
							...
							if(wTableID==INVALID_TABLE || wTableID>MAX_TABLE)
								continue;
							bool bSuccess=true;
...
bool  CGameMatchServiceManager::RectifyServiceOption(tagGameServiceOption * pGameServiceOption,tagGameServiceAttrib *pGameServiceAttrib)
{
	...
	{
		m_ppITableFrame=new ITableFrame*[MAX_TABLE];
		m_bTableUsed=new bool[MAX_TABLE];
	}
	...
	//构造路径
	CString szFileName;
	szFileName.Format(TEXT("%s\\MatchOption%d.ini"),szPath,m_pGameServiceOption->dwServerID);
...
bool CGameMatchServiceManager::SendGameMessage(IServerUserItem * pIServerUserItem, LPCTSTR lpszMessage, WORD wMessageType)
{
	...
	//发送数据
	WORD wSendSize=sizeof(Message)-sizeof(Message.szString)+Message.wLength*sizeof(TCHAR);
	return m_pIGameServiceFrame->SendData(pIServerUserItem,MDM_G_FRAME,CMD_GF_SYSTEM_MESSAGE,&Message,wSendSize);
...
bool CGameMatchServiceManager::AssignTable(CChampionshipMatch *pMatch, WORD wTableCount)
{
	...
	WORD n=0,wCount=0;
	while((wCount<wTableCount)&&n<MAX_TABLE)
	{
		if(!m_bTableUsed[n])
		{
			bool bEmpty=true;
	...
	if(n>MAX_TABLE || wCount<wTableCount)
	{
...
bool CGameMatchServiceManager::WriteUserAward(IServerUserItem *pIServerUserItem, WORD wRank,DWORD dwMatchNO,LPCTSTR szDescribe,CChampionshipMatch *pMatch)
{
	...
	if(wRank<m_RewardArr.GetCount())
	{
		if(pIServerUserItem!=NULL)
		{
			...
			//发送数据
			m_pIGameServiceFrame->SendData(pIServerUserItem, MDM_G_FRAME, SUB_GR_MATCH_RESULT, &MatchResult, sizeof(MatchResult));
...
bool CGameMatchServiceManager::SendGroupUserScore(IServerUserItem * pIServerUserItem,CChampionshipMatch *pMatch)
{
	...
	if(pMatch!=NULL)
	{
		...
		while(pos!=NULL)
		{
			...
			m_pIGameServiceFrame->SendData(pUserItem,MDM_USER,SUB_GR_USER_SCORE,&UserScore,sizeof(UserScore));	
		}
		...
		m_pIGameServiceFrame->SendData(BG_COMPUTER,MDM_USER,SUB_GR_USER_SCORE,&UserScore,sizeof(UserScore));
...
bool  CGameMatchServiceManager::SendGroupUserStatus(IServerUserItem * pIServerUserItem,CChampionshipMatch *pMatch)
{
	...
	while(pos!=NULL)
	{
		...
		if(pUserItem->IsClientReady())
			m_pIGameServiceFrame->SendData(pUserItem,MDM_USER,SUB_GR_USER_STATUS,&UserStatus,sizeof(UserStatus));	
	}

	return true;
}
//用户比赛成绩
bool CGameMatchServiceManager::OnUserMatchResult(DWORD dwUserID, WORD wKindID, DWORD wServerID, WORD wSortID)
{
	return true;
}
	...
	case DBO_GR_MATCH_FEE_RESULT:		//报名结果
		{
			...
			else if(lReturnValue==1)
			{
				...
				_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("您游戏币不够，不能加入比赛！"));
	...
	case DBO_GR_MATCH_QUIT_RESULT:		//退赛结果
		{
			...
			{
				TCHAR szDescribe[128]=TEXT("");
				_sntprintf_s(szDescribe,CountArray(szDescribe),TEXT("退赛成功，退还报名费%d游戏币！"),m_MatchRule.dwFee);
				m_pIGameServiceFrame->SendRoomMessage(pIServerUserItem,szDescribe,SMT_EJECT);				
			}
			break;
		}	
