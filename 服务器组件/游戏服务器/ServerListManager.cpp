...
CServerListManager::~CServerListManager()
{
	...
	while (Position!=NULL)
	{
		m_ServerItemMap.GetNextAssoc(Position,RoomKey,pGameServerItem);
...
bool CServerListManager::InsertGameServer(tagGameServer * pGameServer)
{
	...
	{
		//保存变量
		wKindID = pGameServerItem->m_GameServer.wServerID >> 16;
	...
	//设置人数
	WORD GameKindID = pGameServerItem->m_GameServer.wServerID >> 16;
	if (wKindID != GameKindID)
	{
		//变量定义
		...
		//当前人数
		if (m_KindItemMap.Lookup(GameKindID, pGameKindItemCurrent)==TRUE)
		{
...
//删除节点
bool CServerListManager::DeleteGameNode(WORD wNodeID, WORD wKindID)
{
	//查找节点
	DWORD key = (wKindID << 16) + (wNodeID<<8);
	CGameNodeItem * pGameNodeItem=NULL;

	//TODONOW added by WangChengQing 此处强转会导致问题
	if (m_NodeItemMap.Lookup(key,pGameNodeItem)==FALSE) return false;

	//删除数据
	m_NodeItemMap.RemoveKey(key);
	m_NodeItemBuffer.Add(pGameNodeItem);
...
bool CServerListManager::DeleteGameServer(DWORD wServerID)
{
	...
	CGameKindItem * pGameKindItem=NULL;
	WORD GameKindID = pGameServerItem->m_GameServer.wServerID >> 16;
	if (m_KindItemMap.Lookup(GameKindID, pGameKindItem)==TRUE)
	{
		tagGameServer * pGameServer=&pGameServerItem->m_GameServer;
		//pGameKindItem->m_GameKind.dwOnLineCount-=pGameServer->dwOnLineCount;
		//pGameKindItem->m_GameKind.dwFullCount-=pGameServer->dwFullCount;
