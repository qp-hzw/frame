...
typedef CMap<DWORD,DWORD,CGlobalServerItem *,CGlobalServerItem *> CMapServerID;
...
public:
	...
	//获取类型
	WORD GetKindID() { return ((m_GameServer.wServerID>>16)); }
	//获取房间
	DWORD GetServerID() { return m_GameServer.wServerID; }
	//用户数目
...
public:
	//删除用户
	bool DeleteUserItem(DWORD dwUserID, DWORD wServerID);
	//激活用户
	bool ActiveUserItem(tagGlobalUserInfo & GlobalUserInfo, DWORD wServerID);
...
public:
	//删除房间
	bool DeleteServerItem(DWORD wServerID);
	//激活房间
...
public:
	...
	//寻找房间
	CGlobalServerItem * SearchServerItem(DWORD wServerID);
